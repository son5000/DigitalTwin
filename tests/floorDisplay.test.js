...
const FLOORS = [
  { id: "floor-b2", level: -2, elevation: -7.2, name: "지하 기계실" },
  { id: "floor-b1", level: -1, elevation: -3.6, name: "지하 주차장" },
...
test("층 이동 목록은 안정적인 ID를 유지하며 층 번호 순으로 정렬한다", () => {
  assert.deepEqual(sortFloorsByLevel(FLOORS).map((floor) => floor.id), ["floor-b2", "floor-b1", "floor-1", "floor-2", "floor-3"]);
  assert.equal(formatFloorOptionLabel(FLOORS[2]), "3F");
  assert.equal(formatFloorOptionLabel(FLOORS[1]), "B1");
});

test("표시 간격은 층 순서에만 곱하고 실제 elevation을 변경하지 않는다", () => {
  const offsets = createFloorDisplayOffsets(FLOORS, 2.5);
  assert.deepEqual([...offsets], [["floor-b2", -5], ["floor-b1", -2.5], ["floor-1", 0], ["floor-2", 2.5], ["floor-3", 5]]);
  assert.deepEqual(FLOORS.map((floor) => floor.elevation), [-7.2, -3.6, 7.2, 0, 3.6]);
});
