...
export default function MonitoringSettingsPanel({
  ...
  return <section className={styles.panel}>
    <header><span>EQUIPMENT DETAIL · {selectedEquipment.id}</span><h2>설비 상세</h2><p>{selectedEquipment.name}의 Proxy, 실제 스캔 자산, 현실 센서와 서버 키를 연결합니다.</p></header>
    ...
    {tab === "BASIC" ? <div className={styles.content}>
      <dl className={styles.summary}><div><dt>설비 ID</dt><dd>{selectedEquipment.id}</dd></div><div><dt>템플릿</dt><dd>{selectedEquipment.shapeTemplateId}</dd></div><div><dt>층</dt><dd>{selectedEquipment.floorId}</dd></div><div><dt>연결 현황</dt><dd>자산 {scopedAssets.length} · 센서 {scopedSensors.length} · 포인트 {scopedPoints.length}</dd></div></dl>
      <p className={styles.help}>Proxy Model은 설비 배치와 충돌 판정의 기준으로 유지하며 실제 자산은 별도 정합 transform으로만 보정됩니다.</p>
    </div> : null}

    {tab === "ASSET" ? <div className={styles.content}>
      <div className={styles.actionRow}><label className={styles.uploadButton}>사용자 파일 연결<input type="file" accept=".obj,.ply,.jpg,.jpeg,.png" onChange={uploadFile} /></label><button type="button" onClick={() => onAddAsset(selectedEquipment.id, { name: "서버 자산", sourceType: ASSET_SOURCE_TYPES.SERVER_KEY, sourceKey: "asset/cabinet/latest", assetType: ASSET_TYPES.OBJ, usageType: ASSET_USAGE_TYPES.MODEL, status: "PENDING" })}>서버 asset key 추가</button></div>
      <div className={styles.itemList}>{scopedAssets.map((asset) => <button type="button" key={asset.id} aria-pressed={selectedAsset?.id === asset.id} onClick={() => onSelectAsset(asset.id)}><span>{asset.name}</span><small>{asset.assetType} · {asset.usageType} · {asset.status}</small></button>)}</div>
      {selectedAsset ? <><label className={styles.field}><span>사용 용도</span><select value={selectedAsset.usageType} onChange={(event) => onUpdateAsset(selectedAsset.id, { usageType: event.target.value })}>{Object.values(ASSET_USAGE_TYPES).map((value) => <option key={value}>{value}</option>)}</select></label><label className={styles.field}><span>보기 모드</span><select value={selectedAsset.displayMode} onChange={(event) => onUpdateAsset(selectedAsset.id, { displayMode: event.target.value })}>{Object.entries({ PROXY: "Proxy만", ACTUAL: "실제 모델만", COMPARE: "Proxy와 실제 모델 비교", POINT_CLOUD: "포인트클라우드" }).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className={styles.field}><span>Source Key / URL</span><input value={selectedAsset.sourceKey} onChange={(event) => onUpdateAsset(selectedAsset.id, { sourceKey: event.target.value })} /></label></> : <p className={styles.help}>연결된 자산을 선택하세요.</p>}
    </div> : null}
    ...
    {tab === "SERVER" ? <div className={styles.content}>
      ...
      <div className={styles.itemList}>{scopedServers.map((binding) => <button type="button" key={binding.id} aria-pressed={selectedServer?.id === binding.id} onClick={() => onSelectServer(binding.id)}><span>{binding.metric}</span><small>{binding.protocol} · {binding.serverKey || "key 미지정"}</small></button>)}</div>
      {selectedServer ? <><label className={styles.field}><span>Server Key</span><input value={selectedServer.serverKey ?? ""} onChange={(event) => onUpdateServer(selectedServer.id, { serverKey: event.target.value })} /></label><label className={styles.field}><span>프로토콜</span><select value={selectedServer.protocol} onChange={(event) => onUpdateServer(selectedServer.id, { protocol: event.target.value })}>{PROTOCOLS.map((value) => <option key={value}>{value}</option>)}</select></label><label className={styles.field}><span>Endpoint</span><input value={selectedServer.endpoint} onChange={(event) => onUpdateServer(selectedServer.id, { endpoint: event.target.value })} /></label><label className={styles.field}><span>Topic / Path</span><input value={selectedServer.topicOrPath} onChange={(event) => onUpdateServer(selectedServer.id, { topicOrPath: event.target.value })} /></label><div className={styles.preview}><span>백엔드 미연결 데모</span><strong>42.3 {selectedServer.unit}</strong><code>{`{ "assetKey": "${selectedServer.serverKey}", "value": 42.3 }`}</code></div></> : null}
    </div> : null}
    ...
    {tab === "SENSOR" ? <div className={styles.content}>
      ...
      <div className={styles.itemList}>{scopedSensors.map((sensor) => <button type="button" key={sensor.id} aria-pressed={selectedSensor?.id === sensor.id} onClick={() => onSelectSensor(sensor.id)}><span>{sensor.name}</span><small>{sensor.sensorType} · {sensor.serverKey || "key 미지정"}</small></button>)}</div>
      {selectedSensor ? <><label className={styles.field}><span>이름</span><input value={selectedSensor.name} onChange={(event) => onUpdateSensor(selectedSensor.id, { name: event.target.value })} /></label><label className={styles.field}><span>Server Key</span><input value={selectedSensor.serverKey} onChange={(event) => onUpdateSensor(selectedSensor.id, { serverKey: event.target.value })} /></label><label className={styles.field}><span>장착 방식</span><select value={selectedSensor.mountMode} onChange={(event) => onUpdateSensor(selectedSensor.id, { mountMode: event.target.value })}><option value="WORLD">월드 고정</option><option value="EQUIPMENT">설비 부착</option></select></label><VectorFields label="위치 m" value={selectedSensor.position} onChange={(position) => onUpdateSensor(selectedSensor.id, { position })} /><VectorFields label="회전 rad" value={selectedSensor.rotation} onChange={(rotation) => onUpdateSensor(selectedSensor.id, { rotation })} />{selectedSensor.sensorType === "CAMERA" ? <div className={styles.rangeGrid}><NumericField label="화각" value={selectedSensor.fieldOfView} min={10} unit="°" onChange={(fieldOfView) => onUpdateSensor(selectedSensor.id, { fieldOfView })} /><NumericField label="종횡비" value={selectedSensor.aspectRatio} min={0.2} onChange={(aspectRatio) => onUpdateSensor(selectedSensor.id, { aspectRatio })} /><NumericField label="Near" value={selectedSensor.near} min={0.01} unit="m" onChange={(near) => onUpdateSensor(selectedSensor.id, { near })} /><NumericField label="Far" value={selectedSensor.far} min={0.1} unit="m" onChange={(far) => onUpdateSensor(selectedSensor.id, { far })} /></div> : null}</> : null}
    </div> : null}
    ...
    {tab === "POINT" ? <div className={styles.content}>
      ...
      <div className={styles.itemList}>{scopedPoints.map((point) => <button type="button" key={point.id} aria-pressed={selectedPoint?.id === point.id} onClick={() => onSelectPoint(point.id)}><span>{point.name}</span><small>{point.metric} · 센서 {point.sensorIds.length}</small></button>)}</div>
      {selectedPoint ? <><label className={styles.field}><span>이름</span><input value={selectedPoint.name} onChange={(event) => onUpdatePoint(selectedPoint.id, { name: event.target.value })} /></label><label className={styles.field}><span>관측 항목</span><select value={selectedPoint.metric} onChange={(event) => onUpdatePoint(selectedPoint.id, { metric: event.target.value })}>{METRICS.map((value) => <option key={value}>{value}</option>)}</select></label><VectorFields label="설비 로컬 위치" value={selectedPoint.localPosition} onChange={(localPosition) => onUpdatePoint(selectedPoint.id, { localPosition })} /><VectorFields label="표면 법선" value={selectedPoint.targetNormal} onChange={(targetNormal) => onUpdatePoint(selectedPoint.id, { targetNormal })} /></> : null}
    </div> : null}
