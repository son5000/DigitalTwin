...
const TABS = [["BASIC", "기본 정보"], ["SENSOR", "센서·카메라"], ["ASSET", "실제 데이터"], ["POINT", "관측 포인트"], ["SERVER", "서버 연결"], ["ALIGN", "정합·보정"]];
...
export default function MonitoringSettingsPanel({
  ...
  onUpdateEquipment, onUploadAssetFiles,
}) {
  ...
  function uploadFile(event) {
    ...
    onUploadAssetFiles?.(selectedEquipment.id, files);
    event.target.value = "";
  }

  return <section className={styles.panel}>
    <header><span>선택 설비 · {selectedEquipment.id}</span><h2>{selectedEquipment.name}</h2><p>센서·카메라 → 실제 데이터 → 관측 포인트 → 서버 연결 순서로 설정합니다.</p></header>
