const DATABASE_NAME = "digital-twin-texture-assets";
...
function openDatabase() {
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () => reject(request.error ?? new Error("텍스처 저장소를 열 수 없습니다"));
  });
}
...
async function transact(mode, callback) {
  ...
  try {
    return await new Promise((resolve, reject) => {
      ...
      request.onerror = () => reject(request.error ?? new Error("텍스처 저장 작업에 실패했습니다"));
    });
  } finally {
    database.close();
  }
...
export const textureAssetRepository = {
